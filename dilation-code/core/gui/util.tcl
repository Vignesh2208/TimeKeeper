...
proc popupDilationConfig { } {
    ...
    wm transient $wi .
    wm resizable $wi 1 1
    wm title $wi "Dilation Factor"

    # help text at top
    frame $wi.top
    set helptext "Set the dilation factor for the entire experiment\n"
    set helptext "$helptext 0 will run at normal speed\n"
    set helptext "$helptext 2 will make a process run twice as fast\n"
    set helptext "$helptext -2 will make a process run half as slow\n"
    ...
    set dil_start 0
    frame $wi.dilg
    label $wi.dilg.dillab -text "Default Dilation Factor:"
    entry $wi.dilg.name -width 5 -bg white -textvariable dil_start
    pack $wi.dilg.dillab $wi.dilg.name $wi.dilg -side left
    pack $wi.dilg -side top

    ttk::frame $wi.butt -borderwidth 6
    ttk::button $wi.butt.apply -text "Apply" -command "dilApply $wi"
    focus $wi.butt.apply
    pack $wi.butt.apply -side left
    pack $wi.butt
...
proc dilApply { wi } {
    global node_list
    set dilation [string trim [$wi.dilg.name get]]
    foreach node $node_list {
        setNodeDilation $node $dilation
    }
    popdownConfig $wi
}
