...
pid_t nsfork(int flags, int dilation)
{
  int pid;
  char tk_addToExp[100];
  char tk_dilate[100];
  ...
  if (pid == 0)                 /* child */
  {
    ...
    if (dilation < 0)
        sprintf(tk_dilate,"B,%d,1,%d\n",pid,dilation*-1);
    else
        sprintf(tk_dilate,"B,%d,%d\n",pid,dilation);
    fprintf(fp, "%s", tk_dilate);
    fclose(fp);
    fp = fopen("/proc/dilation/status", "a");
    sprintf(tk_addToExp,"F,%d\n",pid);
    fprintf(fp, "%s", tk_addToExp);
    fclose(fp);
  }
  return pid;
}
