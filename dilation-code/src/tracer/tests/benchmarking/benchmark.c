...
int run_commanded_process(llist * active_tids, pid_t pid, int n_instructions, int disable_buffer_window){
	...
	while(1) {
		...
		if(singlestepmode) {
			...
			if(!disable_buffer_window)
				...
				ptrace(PTRACE_SET_DELTA_BUFFER_WINDOW, pid, 0, (unsigned long *)&buffer_window_size);
			}
			pd = libperf_initialize((int)pid,0); /* init lib */
...
int main(int argc, char * argv[]){
	...
	while(n_cmds < 100) {
		...
		measurements[measurement_idx].std_overshoot_err += (((float)ret1*ret1));
		measurements[measurement_idx].avg_elapsed_time_window += (float)micros_used;
		measurements[measurement_idx].std_elapsed_time_window += ((float)micros_used*micros_used);
    	...
    	measurements[measurement_idx].avg_elapsed_time_nowindow += (float)micros_used;
		measurements[measurement_idx].std_elapsed_time_nowindow += ((float)micros_used*micros_used);
