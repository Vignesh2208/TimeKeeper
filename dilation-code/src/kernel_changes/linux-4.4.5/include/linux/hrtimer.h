...
struct hrtimer_dilated_clock_base {
	struct hrtimer_cpu_base	*cpu_base;
	int clock_active;	//indicates whether dilated clock is active i.e whether TimeKeeper experiment is running. If not new dilated hrtimer init should fail and existing hrtimer callbacks should end.
	struct timerqueue_head	active;
	ktime_t	 (*get_time)(void);
} __attribute__((__aligned__(HRTIMER_CLOCK_BASE_ALIGN)));
