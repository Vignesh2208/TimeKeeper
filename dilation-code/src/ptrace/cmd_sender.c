...
void send_command(int sockfd, struct msghdr * msg, struct nlmsghdr * nlh, int ptrace_loader_pid, int process_pid, int n_instructions){

	sprintf(NLMSG_DATA(nlh),"%d %d", process_pid, n_instructions);
...
int main(int argc, char * argv[]){
	...
	int n_instructions = 0;
        ...
        if (argc < 4 || !strcmp(argv[1], "-h") || !strcmp(argv[1], "--help")) {
                ...
                fprintf(stderr, "       %s PTRACE_LOADER_PID PROCESS_PID N_INSTRUCTIONS\n", argv[0]);
                fprintf(stderr, "\n");
	...
	process_pid = atoi(argv[2]);
	n_instructions = atoi(argv[3]);

	printf("Ptrace loader: %d, Process to control: %d, number of instructions: %d\n", ptrace_loader_pid, process_pid, n_instructions);
	...
	send_command(sockfd, &msg,nlh, ptrace_loader_pid, process_pid, n_instructions); 
