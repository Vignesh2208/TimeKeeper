...
SYSCALL_DEFINE1(time, time_t __user *, tloc)
{
	...
	if(current->virt_start_time != 0){
                        s64 now = i*1000000000;
                        s32 rem;
                        s64 real_running_time;
                        s64 dilated_running_time;
                        real_running_time = now - current->virt_start_time;
                        if (current->dilation_factor > 0) {
                                dilated_running_time = div_s64_rem(real_running_time - current->past_physical_time ,current->dilation_factor,&rem) + current->past_virtual_time;
                                now = dilated_running_time + current->virt_start_time;
                        }
                        else if (current->dilation_factor < 0) {
                                dilated_running_time = (real_running_time - current->past_physical_time)*(current->dilation_factor*-1) + current->past_virtual_time;
                                now =  dilated_running_time + current->virt_start_time;
                        }
                        else {
        		...
        		i = div_s64_rem(now ,1000000000,&rem);
	}
	*/
	if (tloc) {
		if (put_user(i,tloc))
